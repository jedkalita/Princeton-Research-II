...
def fit_lstm(train, batch_size, nb_epoch, neurons):
    ...
    for i in range(nb_epoch):
        print("epoch number for training: %d" %(i))
...
for i in range(len(diff_values)):
    value = inverse_difference(dataset, diff_values[i], len(dataset) - i)
...
train_size = int(len(supervised_values) * 0.70)
test_size = len(supervised_values) - train_size
'''print(train_size)
print(test_size)'''
train, test = supervised_values[0:train_size,:], supervised_values[train_size:len(supervised_values),:]
#print(len(train), len(test)) #9398 training set size, 4630 test set size
